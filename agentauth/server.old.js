...
const allowedOrigins = [
  ...
  'https://agent-identity-production.up.railway.app' // Railway dashboard
];
...
app.get('/agents', authenticateToken, async (req, res) => {
  try {
    ...
    const { data: agents, error } = await supabase
      .from('agents')
      .select('agent_id, name, description, owner_email, permissions, status, tier, created_at, last_verified_at')
...
app.get('/permissions/list', async (req, res) => {
  ...
  for (const [service, permissions] of Object.entries(VALID_PERMISSIONS)) {
    ...
    permissionsFlat.push(...formatted);
  }
